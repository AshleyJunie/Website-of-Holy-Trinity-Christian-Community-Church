...
// This page's ministry label (for the form and UI)
$pageMinistryLabel   = "Handmaid's of the Lord";
// Optional parent label
$parentMinistryType  = "Women Ministry";

// Fetch all Handmaid’s of the Lord content
$stmt = $pdo->prepare("SELECT img_caption, img_file_path, join_desc 
                       FROM content_management_table 
                       WHERE content_type = 'Handmaid' 
                       ORDER BY contentID ASC");
$stmt->execute();
$handmaids = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Default image
$image = 'image/ministries-women-last.png';

// Collect captions + join_desc
$captions = [];
$joinDesc = '';
foreach ($handmaids as $row) {
...
// Progressive fallback target depending on login state
...
/* ================== Fetch gender, birthday & baptism_verification in ONE query ================== */
$userGender        = null;
$userAge           = null;
...
if (!empty($_SESSION['individual_id'])) {
    try {
        $stmtInd = $pdo->prepare("
            SELECT individual_gender, individual_birthday, baptism_verification
            FROM individual_table
            WHERE individual_id = :id
            LIMIT 1
        ");
        $stmtInd->execute([':id' => (int)$_SESSION['individual_id']]);
        ...
        if ($indRow) {
            ...
            if (isset($indRow['baptism_verification'])) {
                $userBaptismStatus = $indRow['baptism_verification'];
            }
        }
    } catch (Exception $e) {
        // Fail silently
...
/* ================== Check existing Handmaid's of the Lord membership ================== */
$isHandmaidMinistryMember = false;
...
if (!empty($_SESSION['individual_id'])) {
    try {
        ...
        $isHandmaidMinistryMember = (bool)$stmtMin->fetchColumn();
    } catch (Exception $e) {
...
<head>
    ...
    <title>Ministry - Handmaid's of the Lord</title>
    ...
    <script>
      ...
      const IS_HANDMAID_MEMBER = <?php echo $isHandmaidMinistryMember ? 'true' : 'false'; ?>;
    </script>
...
<section id="service-preach">
  <!-- Match preach structure: big header + card -->
  <h2 class="service-header">MINISTRIES</h2>
  <div class="service-container">
  <div class="service-text">
    <h3 class="service-header-txt-preach">HANDMAID'S OF THE LORD</h3>

    <!-- MOBILE/TABLET IMAGE: directly under the header -->
    <div class="service-image-container mobile-only">
      <div class="service-image">
        <img src="<?php echo htmlspecialchars($image); ?>" alt="Handmaid’s of the Lord">
      </div>
    </div>
    ...
    <div class="background" id="background">
      <a href="<?php echo htmlspecialchars($fallbackHref); ?>" class="read-more-btn-baptism" id="joinNowBtn">
        JOIN NOW!
      </a>
    </div>
  </div>
  ...
  <div class="service-image-container desktop-only">
    <div class="service-image">
      <img src="<?php echo htmlspecialchars($image); ?>" alt="Handmaid’s of the Lord" id="service-img">
    </div>
  </div>
</div>
...
<!-- SweetAlert gating for JOIN NOW! (NOT-LOGGED-IN users) -->
<script>
(function(){
...
<!-- Handmaid's duplicate-membership gating when clicking JOIN NOW (logged-in) -->
<script>
(function(){
  const btn = document.getElementById('joinNowBtn');
  if (!btn) return;
  ...
  btn.addEventListener('click', function(e){
    ...
    if (IS_HANDMAID_MEMBER) {
      e.preventDefault();
      Swal.fire({
        icon: 'info',
        title: 'You are already part of this ministry',
        text: 'Our records show that you are already a member of the Handmaid\'s of the Lord ministry.'
      });
      return;
    }
  });
...
<script>
(function(){
  if (!IS_LOGGED_IN || !IS_HANDMAID_MEMBER) return;
...
<!-- Gender & age gating for logged-in users -->
<script>
(function(){
  const btn = document.getElementById('joinNowBtn');
  if (!btn) return;
  ...
  btn.addEventListener('click', function(e){
    ...
    if (genderRaw === 'male' || genderRaw === 'm') {
      ...
      Swal.fire({
        icon: 'warning',
        title: 'Women’s Ministry Only',
        text: 'This ministry is exclusively for women. You will not be able to proceed with this form using a male account.'
      });
      return;
    }
    ...
    if (!Number.isNaN(ageParsed) && ageParsed < 18) {
      ...
      Swal.fire({
        icon: 'warning',
        title: 'Not Applicable for Minors',
        text: 'This ministry is not applicable for minors. Only members 18 years old and above may join.'
      });
      return;
    }
  });
...
<!-- baptism_verification gating (logged-in) -->
<script>
(function(){
  const btn = document.getElementById('joinNowBtn');
  if (!btn) return;
...
<!-- baptism_verification error / reupload trap -->
<script>
(function(){
  const btn = document.getElementById('joinNowBtn');
  if (!btn) return;
  ...
  btn.addEventListener('click', function(e){
    ...
    Swal.fire({
      icon: 'warning',
      title: 'Baptismal Verification Issue',
