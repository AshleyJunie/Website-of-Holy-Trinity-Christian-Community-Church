...
<aside class="sidebar">
  ...
  <nav class="nav">
    ...
    <a class="navlink active" href="pastor-men-ministries.php">
      ...
      <tbody>
        ...
        if ($result && mysqli_num_rows($result) > 0) {
          while ($row = mysqli_fetch_assoc($result)) {
            ...
            /* ADDED: data-mid carries the real primary key for reliable editing */
            echo '<td>
...
(function(){
  ...
  function getMainTable(){
    // first table within the page container
  ...
  function ensureWrapper(table){
    ...
    // insert before, then move table inside
...
<!-- ==========================================================
     ADDED: EDIT FEATURE (buttons + modal + ajax) for MEN
========================================================== -->
<script>
(function(){
  ...
  async function attachEditors(){
    ...
    rows.forEach(tr=>{
      ...
      btn.addEventListener('click', async ()=>{
        let id = btn.getAttribute('data-mid'); // NOW PRESENT
        if (!id) {
