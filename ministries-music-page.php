...
// This page's ministry label (used for form target + membership check)
$pageMinistryLabel = "Music Ministry";

// Fetch all Music Ministry data
...
// Default image
$image = 'image/ministries-music-last.png';
...
// Always include the ministry in the target (must match ministry_type label)
...
if (!empty($_SESSION['individual_id'])) {
    try {
        ...
        // Silent fail: if something goes wrong, baptism gating will just not trigger.
    }
}

/* ================== Check existing Music Ministry membership ================== */
$isMusicMinistryMember = false;
...
if (!empty($_SESSION['individual_id'])) {
    try {
        ...
        $isMusicMinistryMember = (bool)$stmtMin->fetchColumn();
    } catch (Exception $e) {
...
<head>
    ...
    <title>Ministry - Music's Ministries</title>
    ...
    <link rel="stylesheet" href="/HTCCC-SYSTEM/css/ministries-music-page.css?v=<?php echo filemtime($_SERVER['DOCUMENT_ROOT'].'/HTCCC-SYSTEM/css/ministries-music-page.css'); ?>">
    <!-- SweetAlert2 -->
    ...
    <script>
      ...
      const IS_MUSIC_MINISTRY_MEMBER = <?php echo $isMusicMinistryMember ? 'true' : 'false'; ?>;
    </script>
...
<section id="service-preach">
    <!-- Follow Handmaid/preach pattern: big header + card + subheading -->
    <h2 class="service-header">MINISTRIES</h2>

    <div class="service-container">
        <div class="service-text">
            <h3 class="service-header-txt-preach">MUSIC'S MINISTRIES</h3>

            <!-- MOBILE/TABLET IMAGE: directly under the header -->
            <div class="service-image-container mobile-only">
              <div class="service-image">
                <img src="<?php echo htmlspecialchars($image); ?>" alt="Music Ministry">
              </div>
            </div>
        ...
        <div class="service-image-container desktop-only">
            <div class="service-image">
                <img src="<?php echo htmlspecialchars($image); ?>" alt="Music Ministry" id="service-img">
            </div>
        </div>
    </div>
</section>
...
<!-- Duplicate-membership gating for Music Ministry (LOGGED-IN ONLY) -->
<script>
(function(){
  const btn = document.getElementById('joinNowBtn');
  if (!btn) return;
  ...
  btn.addEventListener('click', function(e){
    ...
    if (IS_MUSIC_MINISTRY_MEMBER) {
      e.preventDefault();
      Swal.fire({
        icon: 'info',
        title: 'You are already part of this ministry',
        text: 'Our records show that you are already a member of the Music Ministry.'
      });
      return;
    }
  });
...
<script>
(function(){
  if (!IS_LOGGED_IN || !IS_MUSIC_MINISTRY_MEMBER) return;
