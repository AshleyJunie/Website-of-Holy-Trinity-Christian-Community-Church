...
/* =============================================================================
   === ADD BELOW THIS LINE: SESSION & AUDIT TRAIL HELPERS (NO DESIGN CHANGES) ===
   - Mirrors the Women’s Ministry audit logic:
     * JSON insert via prepared stmt → retry once → minimal raw insert
     * UUID txn_id; actor from $_SESSION; IP (XFF-aware); user agent
   - Used when Approve/Decline succeeds
   ========================================================================== */
if (session_status() !== PHP_SESSION_ACTIVE) { @session_start(); }

if (!function_exists('audit_ministry_action')) {
  /**
   * Insert into audit_trail with robust fallbacks.
   * @param mysqli $db
   * @param string $action  e.g., 'APPROVE' | 'DECLINE'
   * @param string $recordPk
   * @param array  $detailsAfterArr  JSON payload (current/after state)
   * @param string $notes  human-readable message
   */
  function audit_ministry_action(
    ...
    $form   = 'application_ministry.php';
    $source = 'ministries_table';

    if ($notes === '') {
      $notes = in_array($action, ['APPROVE','DECLINE'], true)
        ? "Ministry application decision — {$action}"
        : "Ministry application action — {$action}";
    }
    ...
    // Attempt #3: minimal raw insert (no JSON columns)
    ...
    $notesE = $db->real_escape_string($notes);
    $sql3 = "
      ...
      (UUID(), {$actorId}, '{$u}', '{$e}', '{$action}', '{$source}',
       '{$pk}', '{$form}', '{$ipEsc}', '{$uaEsc}', '{$notesE}', NULL, NULL)";
...
/** Build readable decision notes (Pending → Approved/Declined). */
if (!function_exists('build_decision_notes')) {
  function build_decision_notes(array $ctx): string {
    $id     = $ctx['id']     ?? '';
    $name   = trim(($ctx['lname'] ?? '') . ', ' . ($ctx['fname'] ?? ''));
    $type   = $ctx['type']   ?? '';
    $prior  = $ctx['prior']  ?? 'Pending';
    $after  = $ctx['after']  ?? '';
    $emailR = $ctx['email_result'] ?? ''; // 'sent' | 'failed' | 'invalid' | ''

    $emailTxt = $emailR ? "; Email: {$emailR}" : '';
    return "Ministry application decision — ID {$id}; Status: {$prior} → {$after}; Name: {$name}; Type: {$type}{$emailTxt}";
  }
}
/* ============================ END AUDIT HELPERS ADD =========================== */

/* ============================================================================
   NEW: Admin → Individual notification helper for Ministry decisions
   - Creator: always ADMIN (created_by_type = 'admin', created_by_id = admin ID)
   - Receiver: always INDIVIDUAL (user_type = 'individual', user_id = individual_id)
   - Title example:  "Ministry Application – Approved"
   - Body lines:
       Admin: {AdminDisplay}
       Action: {Approved|Declined}
       Service: Ministry Application
       Applied to: {FullName} (Individual ID: {IndividualId})
       Ministry: {MinistryType} (Record ID: {MinistryId})
       Reason: You are receiving this notification because an administrator has
               {approved/declined} your ministry application in the HTCCC System.
       Summary: Ministry Application | Status: {ActionLabel} • Date: {YYYY-MM-DD}
   ============================================================================ */
if (!function_exists('notifyMinistryDecisionToIndividual_mysqli')) {
  /**
   * Create an Admin → Individual notification for a ministry decision.
   *
   * @param mysqli $db
   * @param int    $individualId   Target individual_id (receiver)
   * @param int    $ministryId     ministries_table PK
   * @param string $actionLabel    "Approved" | "Declined"
   * @param string $applicantName  e.g. "Juan Dela Cruz"
   * @param string $ministryType   e.g. "Music Ministry"
   *
   * @return bool
   */
  function notifyMinistryDecisionToIndividual_mysqli(
    mysqli $db,
    int $individualId,
    int $ministryId,
    string $actionLabel,
    ...
    // 1) Creator is always ADMIN
    $createdByType = 'admin';

    // Prefer generic user_id if set, otherwise fallback to admin_id
    ...
    if ($createdById <= 0) {
      // cannot identify admin
      return false;
    }

    // Resolve admin display name (for body only)
    $adminLast  = isset($_SESSION['admin_lastname'])  ? trim((string)$_SESSION['admin_lastname'])  : '';
    $adminFirst = isset($_SESSION['admin_firstname']) ? trim((string)$_SESSION['admin_firstname']) : '';
    ...
    if ($adminLast !== '')  $adminLast  = $titleCase($adminLast);
    if ($adminFirst !== '') $adminFirst = $titleCase($adminFirst);

    if ($adminLast !== '' && $adminFirst !== '') {
      $adminDisplay = "{$adminLast}, {$adminFirst}";
    } elseif ($adminLast !== '') {
      $adminDisplay = $adminLast;
    } elseif ($adminFirst !== '') {
      $adminDisplay = $adminFirst;
    } elseif (!empty($_SESSION['admin_user'])) {
      $adminDisplay = (string)$_SESSION['admin_user'];
    } else {
      $adminDisplay = 'Admin #' . $createdById;
    }

    // Normalize fields
    $serviceLabel  = 'Ministry Application';
    $actionLabel   = trim($actionLabel) !== '' ? trim($actionLabel) : 'Updated';
    $ministryType  = trim($ministryType) !== '' ? trim($ministryType) : 'Ministry';
    $applicantName = trim($applicantName) !== '' ? trim($applicantName) : ('Individual #' . $individualId);

    $verb = (strcasecmp($actionLabel, 'Approved') === 0) ? 'approved' : 'declined';
    $todayYmd = date('Y-m-d');

    // Title
    $title = $serviceLabel . ' – ' . $actionLabel;

    // Body (explicit explanation)
    $lines = [];
    $lines[] = 'Admin: ' . $adminDisplay;
    $lines[] = 'Action: ' . $actionLabel;
    $lines[] = 'Service: ' . $serviceLabel;
    $lines[] = 'Applied to: ' . $applicantName . ' (Individual ID: ' . $individualId . ')';
    $lines[] = 'Ministry: ' . $ministryType . ' (Record ID: ' . $ministryId . ')';
    $lines[] = 'Reason: You are receiving this notification because an administrator has '
              . $verb . ' your ministry application in the HTCCC System.';
    $lines[] = 'Summary: ' . $serviceLabel . ' | Status: ' . $actionLabel . ' • Date: ' . $todayYmd;

    $body = implode("\n", $lines);

    // Truncate safeguard (~2000 chars)
    ...
    $bodyEsc          = mysqli_real_escape_string($db, $body);
    $createdByTypeEsc = mysqli_real_escape_string($db, $createdByType);
    $createdByIdInt   = (int)$createdById;

    $sqlNotif = "
      INSERT INTO notifications (title, body, created_by_type, created_by_id)
      VALUES ('{$titleEsc}', '{$bodyEsc}', '{$createdByTypeEsc}', {$createdByIdInt})
    ";
    if (!mysqli_query($db, $sqlNotif)) {
      return false;
    }

    $notificationId = (int)mysqli_insert_id($db);
    if ($notificationId <= 0) {
      return false;
    }

    // Insert recipient: always Individual
    ...
    if ($id > 0) {
        if ($stmt = mysqli_prepare($db_connection, "UPDATE ministries_table SET status = ? WHERE ministry_id = ? LIMIT 1")) {
            ...
            if ($affected > 0) {
                ...
                // NOTE: expecting ministries_table to have individual_id column
                if ($sel = mysqli_prepare($db_connection, "SELECT ministry_email, ministry_firstname, ministry_lastname, ministry_type, individual_id FROM ministries_table WHERE ministry_id = ? LIMIT 1")) {
                    ...
                    $individualId = isset($row['individual_id']) ? (int)$row['individual_id'] : 0;
                    ...
                    if ($email !== '' && filter_var($email, FILTER_VALIDATE_EMAIL)) {
                        ...
                        /* =============== ADD BELOW THIS LINE: AUDIT CALL (EMAIL VALID) =============== */
                        $emailResult = $ok ? 'sent' : 'failed';
                        $audit_details = [
                          'ministry_id'   => (string)$id,
                        ...
                        $notesCtx = [
                          ...
                          'type' => $mtype, 'prior' => 'Pending', 'after' => $newStatus,
                          'email_result' => $emailResult
                        ];
                        $notes = build_decision_notes($notesCtx);
                        $actionForAudit = ($newStatus === 'Approved') ? 'APPROVE' : 'DECLINE';
                        $audit = audit_ministry_action($db_connection, $actionForAudit, (string)$id, $audit_details, $notes);
                        if (!$audit['ok']) error_log('[AUDIT][ministry-decision]['.$audit['attempt'].'] '.$audit['msg']);
                        /* ============================= END AUDIT CALL ADD ============================ */

                        /* ========== NEW: ADMIN → INDIVIDUAL NOTIFICATION (EMAIL VALID CASE) ========= */
                        if ($individualId > 0 && function_exists('notifyMinistryDecisionToIndividual_mysqli')) {
                          try {
                            notifyMinistryDecisionToIndividual_mysqli(
                              $db_connection,
                              $individualId,         // Individual receiver
                              $id,                   // ministries_table PK
                              $newStatus,            // "Approved" or "Declined"
                              $name,                 // Applicant full name
                              $mtype                 // Ministry type
                            );
                          } catch (Throwable $e) {
                            // Do not block the main action if notification fails.
                            error_log('[NOTIF][ministry-decision] '.$e->getMessage());
                          }
                        }
                        /* ========================= END NEW NOTIFICATION CALL ======================== */

                    } else {
                        $qs['toast'] = urlencode("Application #{$id} set to {$newStatus}. Invalid or missing ministry_email.");
                        $qs['toastType'] = 'warning';

                        /* =============== ADD BELOW THIS LINE: AUDIT CALL (EMAIL INVALID) =============== */
                        $audit_details = [
                          'ministry_id'   => (string)$id,
                          'status_after'  => $newStatus,
                          'status_before' => 'Pending',
                        ...
                        /* ======== NEW: ADMIN → INDIVIDUAL NOTIFICATION (EMAIL INVALID CASE) ======== */
                        if ($individualId > 0 && function_exists('notifyMinistryDecisionToIndividual_mysqli')) {
                          try {
                            notifyMinistryDecisionToIndividual_mysqli(
                              $db_connection,
                            ...
                            error_log('[NOTIF][ministry-decision-invalid-email] '.$e->getMessage());
                          }
                        }
                        /* ========================= END NEW NOTIFICATION CALL ======================== */
                    }
                    ...
                    /* =============== ADD BELOW THIS LINE: AUDIT CALL (LOOKUP FAILED) =============== */
                    $audit_details = [
                      'ministry_id'   => (string)$id,
                      'status_after'  => $newStatus,
                      'status_before' => 'Pending',
                    ...
                    $notes = "Ministry application decision — ID {$id}; Status: Pending → {$newStatus}; (Email lookup failed)";
                    $actionForAudit = ($newStatus === 'Approved') ? 'APPROVE' : 'DECLINE';
                    $audit = audit_ministry_action($db_connection, $actionForAudit, (string)$id, $audit_details, $notes);
                    if (!$audit['ok']) error_log('[AUDIT][ministry-decision]['.$audit['attempt'].'] '.$audit['msg']);
                    /* ============================= END AUDIT CALL ADD ============================ */

                    // No notification here because we don't know which individual to address.
                }
            } else {
...
<aside class="sidebar">
    ...
    <nav class="nav">
      ...
      <a class="navlink active" href="application_ministry.php"><i class="fas fa-users"></i>Ministry Applications</a>
...
<!-- MAIN PAGE -->
<div class="page">
  <header class="topbar"><h1>Online Ministry Applications</h1></header>
...
(function(){
  ...
  // Public-ish: call with 4 rows (kept at 8 per your existing script)
