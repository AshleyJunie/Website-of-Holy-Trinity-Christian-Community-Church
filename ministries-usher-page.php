...
// This page's ministry label (used both for form target and membership check)
$pageMinistryLabel = 'Usher & Usherette';

// DB connect
$pdo = new PDO("mysql:host=localhost;dbname=htccc-data-base;charset=utf8", "root", "");
$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

// Fetch all Usher Ministry data
$stmt = $pdo->prepare("SELECT img_caption, img_file_path, join_desc 
                       FROM content_management_table 
                       WHERE content_type = 'Usher' 
                       ORDER BY contentID ASC");
$stmt->execute();
$usher = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Default image
$image = 'image/ministries-usher-last.png';

// Collect captions + join_desc
$captions = [];
$joinDesc = '';
foreach ($usher as $row) {
    if (!empty($row['img_file_path']) && $row['img_file_path'] !== '0') {
        $image = $row['img_file_path']; // Use DB image if available
    }
...
/* Get age from individual_table to block minors */
$userAge = null;
...
if (!empty($_SESSION['individual_id'])) {
    try {
        ...
        // Silent fail: if something goes wrong, we simply won't enforce the minor-block on client
    }
}
...
/* Check existing Usher & Usherette membership */
$isUsherMinistryMember = false;
...
if (!empty($_SESSION['individual_id'])) {
    try {
        ...
        $isUsherMinistryMember = (bool)$stmtMin->fetchColumn();
    } catch (Exception $e) {
...
<head>
    ...
    <title>Ministry - Usher & Usherette</title>
    ...
    <link rel="stylesheet" href="/HTCCC-SYSTEM/css/ministries-usher-page.css?v=<?php echo filemtime($_SERVER['DOCUMENT_ROOT'].'/HTCCC-SYSTEM/css/ministries-usher-page.css'); ?>">
    <!-- SweetAlert2 -->
    ...
    <script>
      ...
      const IS_USHER_MINISTRY_MEMBER  = <?php echo $isUsherMinistryMember ? 'true' : 'false'; ?>;
    </script>
    ...
    <style>
      ...
      /* Responsive adjustments like preach page */
...
<section id="service-preach">
    ...
    <div class="service-container">
        <div class="service-text">
            <h3 class="service-header-txt-preach">USHER & USHERETTE</h3>

            <!-- MOBILE/TABLET IMAGE: directly under the header -->
            <div class="service-image-container mobile-only">
              <div class="service-image">
                <img src="<?php echo htmlspecialchars($image); ?>" alt="Usher & Usherette Ministry">
              </div>
            </div>
        ...
        <div class="service-image-container desktop-only">
            <div class="service-image">
                <img src="<?php echo htmlspecialchars($image); ?>" alt="Usher & Usherette Ministry" id="service-img">
            </div>
        </div>
    </div>
</section>
...
<!-- Duplicate-membership gating (LOGGED-IN ONLY) -->
<script>
(function(){
  const btn = document.getElementById('joinNowBtn');
  if (!btn) return;
  ...
  btn.addEventListener('click', function(e){
    ...
    if (IS_USHER_MINISTRY_MEMBER) {
      e.preventDefault();
      Swal.fire({
        icon: 'info',
        title: 'You are already part of this ministry',
        text: 'Our records show that you are already a member of the Usher & Usherette ministry.'
      });
      return;
    }
  });
...
<script>
(function(){
  if (!IS_LOGGED_IN || !IS_USHER_MINISTRY_MEMBER) return;
