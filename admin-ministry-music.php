...
/* ✅ Ensure admin FULL NAME in session (Lastname, Firstname Middlename)
   ...
   - still safe if those columns exist (we simply don't use them)
*/
...
if (!function_exists('build_changed_fields_notes')) {
  function build_changed_fields_notes(array $before, array $after): string {
    ...
    if (!$changes) return 'Updated Music Ministry member — No values changed.';
    return 'Updated Music Ministry member — ' . implode('; ', $changes);
  }
}
...
if (!function_exists('audit_ministry_action')) {
  function audit_ministry_action(
    ...
    $form   = 'admin-ministry-music.php';
    $source = 'ministries_table';

    if ($notes === '') {
      $notes = $action==='INSERT'   ? 'Added Music Ministry member'
            : ($action==='UPDATE'   ? 'Updated Music Ministry member'
            : ($action==='ARCHIVE'  ? 'Archived Music Ministry member'
            : ($action==='UNARCHIVE'? 'Unarchived Music Ministry member'
    ...
    $e  = $db->real_escape_string($actorEmail); // will be ''
...
$GLOBALS['__music_edit_old'] = null;
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && (($_POST['__action'] ?? '') === 'edit_ministry_music')) {
  ...
  if ($id > 0 && ($st0 = mysqli_prepare($db_connection, "
    ...
    $GLOBALS['__music_edit_old'] = $res0 ? (mysqli_fetch_assoc($res0) ?: []) : [];
...
register_shutdown_function(function() use ($db_connection) {
  ...
  if (isset($GLOBALS['add_success']) && $GLOBALS['add_success'] === true) {
    ...
    if ($newId !== '0') {
      ...
      $notes = "Added Music Ministry member — Name: {$nameFmt}; Ministry Position: {$position}; Date Joined: {$date_join}";
      $audit = audit_ministry_action($db_connection, 'INSERT', $newId, $audit_details, $notes);
      if (!$audit['ok']) error_log('[AUDIT][music-insert]['.$audit['attempt'].'] '.$audit['msg']);
    }
  }
  ...
  if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && (($_POST['__action'] ?? '') === 'edit_ministry_music')) {
    ...
    if ($id > 0) {
      ...
      $before = $GLOBALS['__music_edit_old'] ?: [];
      if (!empty($after)) {
        ...
        if (!$audit['ok']) error_log('[AUDIT][music-update]['.$audit['attempt'].'] '.$audit['msg']);
      }
    }
  }
});
...
/* Positions for Music Ministry dropdown */
$POSITION_OPTIONS = [
  "Music Director",
  "Assistant Director",
  "Worship Leader",
...
/* =====================================================================
   ADDED ONLY: EDIT FEATURE ENDPOINTS FOR MUSIC MINISTRY (no removals)
===================================================================== */

function music_normalize_date($d){
  if(!$d) return '';
...
if (isset($_GET['ajax']) && $_GET['ajax']==='music_rows') {
  ...
  $q = "SELECT ministry_id, date_join, ministry_position, ministry_lastname, ministry_firstname, ministry_middlename,
        ...
        WHERE ministry_type='Music Ministry' AND archive_status='Active'
...
if (isset($_GET['ajax']) && $_GET['ajax']==='music_row' && isset($_GET['id'])) {
  header('Content-Type: application/json; charset=utf-8');
  $id = (int)$_GET['id'];
  $row = null;
...
if ($_SERVER['REQUEST_METHOD']==='POST' && (($_POST['__action'] ?? '') === 'archive_ministry')) {
  ...
  if ($st = mysqli_prepare($db_connection, $sql)) {
    ...
    if ($ok) {
      ...
      $notes = "Archived Music Ministry member — ID {$id}" .
      ...
      if (!$audit['ok']) error_log('[AUDIT][music-archive]['.$audit['attempt'].'] '.$audit['msg']);
    }
...
if ($_SERVER['REQUEST_METHOD']==='POST' && (($_POST['__action'] ?? '') === 'unarchive_ministry')) {
  ...
  if ($st = mysqli_prepare($db_connection, $sql)) {
    ...
    if ($ok) {
      $lname = $nameRow['ministry_lastname'] ?? '';
      $fname = $nameRow['ministry_firstname'] ?? '';
      $notes = "Unarchived Music Ministry member — ID {$id}" .
               (($lname || $fname) ? " (Name: " . trim($lname . ', ' . $fname, ', ') . ")" : '');
      $audit = audit_ministry_action(
        $db_connection,
        'UNARCHIVE',
      ...
      if (!$audit['ok']) error_log('[AUDIT][music-unarchive]['.$audit['attempt'].'] '.$audit['msg']);
    }
...
/* ===== ADD BELOW THIS LINE — DOMPDF DOWNLOAD (Approved list) =====
   Route: admin-ministry-music.php?download=music_pdf

   ✅ UPDATED:
   - Prepared by: FULL NAME of logged-in admin (from admin_table)
   - NO EMAIL shown
================================================================== */
if (isset($_GET['download']) && $_GET['download'] === 'music_pdf') {
  ...
  $sqlApproved = "
    ...
    FROM ministries_table
    WHERE ministry_type='Music Ministry' AND status='Approved'
    ORDER BY date_join DESC, ministry_lastname, ministry_firstname
  ";
  ...
  $pdfTitle  = "Music Ministry — Approved Members";
  ...
  $fname = 'Music_Ministry_Approved_' . date('Y-m-d_His') . '.pdf';
...
<head>
  ...
  <title>Admin – Music Ministry</title>

  <link rel="icon" type="image/x-icon" href="image/httc_main-logo.jpg">
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">
  <link rel="stylesheet" href="/HTCCC-SYSTEM/css/admin-ministry-music.css?v=<?php echo filemtime($_SERVER['DOCUMENT_ROOT'].'/HTCCC-SYSTEM/css/admin-ministry-music.css'); ?>">
  ...
  <!-- ===== ADD BELOW THIS LINE — SORT PANEL V2 (same UX as Men’s page) CSS ===== -->
  <style>
    .sort2-pop {
...
<aside class="sidebar">
  ...
  <nav class="nav">
    ...
    <a class="navlink active" href="admin-ministry-music.php">
  ...
  <div class="container">
    <div class="top-bar">
      ...
      <div class="btn-group">
        ...
        <!-- Download PDF -->
        <button class="sort-button" type="button" onclick="window.location='admin-ministry-music.php?download=music_pdf'">
          Download PDF
        </button>
        <!-- Archived List -->
...
<div class="modal-backdrop" id="addModal" aria-hidden="true">
  <div class="modal" role="dialog" aria-modal="true" aria-labelledby="addModalTitle">
    <header>
      <h3 id="addModalTitle">Add Music Ministry Member</h3>
    ...
    <form method="post" id="addForm" autocomplete="off">
      ...
      <div class="grid-2" style="margin-top:12px;">
        ...
        <div class="field">
          <label>&nbsp;</label><small>Saved under <b>Music Ministry</b></small>
        </div>
      </div>
...
<div class="modal-backdrop" id="editModal" aria-hidden="true">
  <div class="modal" role="dialog" aria-modal="true" aria-labelledby="editModalTitle">
    <header>
      <h3 id="editModalTitle">Edit Music Ministry Member</h3>
      <button class="close" id="closeEdit" type="button" aria-label="Close">&times;</button>
    </header>

    <form id="editForm" autocomplete="off">
      <input type="hidden" name="__action" value="edit_ministry_music">
...
<div class="modal-backdrop" id="archivedModal" aria-hidden="true">
  <div class="modal" role="dialog" aria-modal="true" aria-labelledby="archivedModalTitle">
    <header>
      <h3 id="archivedModalTitle">Archived Music Ministry Members</h3>
      ...
      <table style="width:100%; border-collapse:collapse;">
        ...
        <tbody>
          ...
          $archivedQuery = "SELECT ministry_id, date_join, ministry_position, ministry_lastname, ministry_firstname, ministry_middlename, ministry_extensionname
                            FROM ministries_table
                            WHERE ministry_type='Music Ministry' AND archive_status='Archived'
                            ORDER BY date_join DESC, ministry_lastname, ministry_firstname";
...
Swal.fire({
  ...
  text: "New member has been added to Music Ministry.",
  icon: "success",
  confirmButtonColor: "#6B5AE3"
}).then(()=>{ window.location = 'admin-ministry-music.php'; });
...
<script>
(function(){
  const WRAP_CLASS = 'js-table-scroll-wrap-music';
...
<!-- ==========================================================
     ADDED: EDIT FEATURE (buttons + modal + ajax) for MUSIC
========================================================== -->
<script>
(function(){
  ...
  let MUSIC_DB_ROWS = null;
  async function ensureDbRows(){
    if (MUSIC_DB_ROWS) return MUSIC_DB_ROWS;
    const res = await fetch('admin-ministry-music.php?ajax=music_rows', { headers: { 'X-Requested-With':'XMLHttpRequest' }});
    const j = await res.json();
    MUSIC_DB_ROWS = (j && j.ok) ? j.rows : [];
    return MUSIC_DB_ROWS;
  }
  ...
  async function attachEditors(){
    ...
    rows.forEach(tr=>{
      ...
      btn.addEventListener('click', async ()=>{
        let id = btn.getAttribute('data-mid'); // PRIMARY KEY directly here
        if (!id) {
        ...
        try{
          const res = await fetch('admin-ministry-music.php?ajax=music_row&id=' + encodeURIComponent(id));
          const j = await res.json();
  ...
  editForm?.addEventListener('submit', async (e)=>{
    ...
    try{
      const res = await fetch('admin-ministry-music.php', { method:'POST', body: fd });
      const j = await res.json();
      if (j && j.ok) {
...
<!-- ===== UNARCHIVE BUTTONS INSIDE ARCHIVED MODAL (NEW) ===== -->
<script>
(function(){
