...
/* =====================================================================
   ...
   - music_rows: list active rows (id + fields) for matching
   - music_row:  fetch a single row by id
   - edit_music: update row by id
===================================================================== */
...
<head>
  ...
  <style>
    ...
    /* ADDED: subtle highlight style for smart search */
...
<aside class="sidebar">
  ...
  <nav class="nav">
    ...
    <a class="navlink active" href="pastor-music-ministries.php">
...
<!-- ==========================================================
     ADDED: SMART SEARCH (fuzzy, tokens, diacritic-insensitive)
     * No changes to existing markup/styles; all runtime only.
========================================================== -->
<script>
(function(){
  ...
  // store original HTML of searchable cells (0..5) for highlighting restore
  rows.forEach(tr=>{
    const orig = [];
  ...
  const INDEX = rows.map(tr=>{
    const t = rowText(tr);
    const init = initials([tr.cells[2]?.innerText,tr.cells[3]?.innerText].join(' ')); // LN FN
    return {tr, text:t, init};
  });

  // "No results" row (added at runtime only)
  let noRow;
  ...
  function matches(entry, tokens){
    ...
    return tokens.every(tok=>{
      ...
      // allow 1 typo for tokens length >= 4
      if (tok.length>=4) {
        // find any word within text with small distance
...
<!-- ==========================================================
     (kept) TABLE HEIGHT LIMIT SCRIPT (duplicate retained as in source)
========================================================== -->
<script>
(function(){
  const WRAP_CLASS = 'js-table-scroll-wrap-music';
